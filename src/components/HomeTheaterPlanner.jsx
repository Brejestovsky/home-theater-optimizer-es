...
function volumePower(roomW, roomL, roomH) {
  ...
  const main = clamp(Math.round((volume * 1.1) / 5) * 5, 30, 150);
  const sub = clamp(Math.round((volume * 2.7) / 5) * 5, 60, 400);
...
export default function HomeTheaterPlanner() {
  ...
  const toRoom = useCallback(
    (px, py) => ({
      x: clamp(Math.round((px - offX) / scale / 5) * 5, 5, roomW - 5),
      y: clamp(Math.round((py - offY) / scale / 5) * 5, 5, roomL - 5),
    }),
  ...
  return (
    <div className="htp-root">
      <style>{`
        ...
        .htp-drag-readout { background: #12283D; border: 1px solid #E8A33D; color: #E8A33D; font-size: 12px; font-weight: 600; border-radius: 6px; padding: 6px 10px; margin-top: 8px; display: inline-block; }
      ...
      <div className="htp-layout">
        ...
        <div className="htp-canvas-wrap">
          ...
          {dragging && (
            <div className="htp-drag-readout">
              {(() => {
                const p = dragging === "LISTEN" ? listen : positions[dragging];
                const ch = config.channels.find((c) => c.id === dragging);
                const name = dragging === "LISTEN" ? "Punto de escucha" : (ch ? ch.label : dragging);
                return `${name} — X: ${Math.round(p.x)} cm, Y: ${Math.round(p.y)} cm`;
              })()}
            </div>
          )}
